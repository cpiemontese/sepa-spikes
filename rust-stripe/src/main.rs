...
async fn create_a_customer(client: &reqwest::Client, stripe_url: &Url, secret_key: &str) -> reqwest::Response {
...
async fn create_a_payment_intent(client: reqwest::Client, stripe_url: Url, secret_key: String, customer_id: &str) -> reqwest::Response {
    let form_data = HashMap::from([
        ("customer".to_string(), customer_id.to_string()),
        ("currency".to_string(), "eur".to_string()),
        ("amount".to_string(), "1000".to_string()),
        ("payment_method_types[]".to_string(), "sepa_debit".to_string()),
        ("metadata[integration_checker]".to_string(), "sepa_debit_accept_a_payment".to_string()),
